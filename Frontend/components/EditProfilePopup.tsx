...
export default function EditProfilePopup({ visible, onClose, onSave, onDeleteAccount, currentName, currentEmail }: Props) {
  ...
  const handleSave = async () => {
    if (name.trim() === '') { Alert.alert('Erro', 'O nome não pode ficar vazio.'); return; }
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.overlay}>
          <View style={styles.popup}>
            ...
            <TextInput style={[styles.input, styles.disabledInput]} placeholder="E-mail" placeholderTextColor="#999" value={currentEmail || ''} editable={false} />
...
const styles = StyleSheet.create({
  ...
  disabledInput: { backgroundColor: '#D3D3D3', color: '#666' },
  buttonRow: { flexDirection: 'row', width: '100%', marginTop: 5 },
